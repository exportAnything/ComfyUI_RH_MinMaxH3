...
- **`res_multistep` 提速约 2.2×**——二阶指数积分器，约 21 个 sigma 点（20 次 DiT）
  即可达到 50 步 Euler 的质量。同一份权重，无需重新量化；默认仍是 `euler`。
...
- **`res_multistep` 采样模式（提速约 2.2×）。** 视频与音频跑在不同的 shift 调度上，
  现在各自在自身调度上做二阶指数积分。约 21 个 sigma 点（20 次 DiT）即可达到
  50 步 Euler 的质量。单卡 832×480/125 帧实测：**548 秒 → 250 秒**，同 seed 下
  无可见质量退化。在采样器的 `sampler_mode` 选择该模式并把 `sigma_points` 设为
  21 即可；默认仍是 `euler`，存量工作流不受影响。该模式会强制 `accel=off`——
  velocity-cache 与 Cache-DiT 的档位按 50 步标定，20 步上会过度跳步。
