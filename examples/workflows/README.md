...
shift `12/3`, `accel=off`, and `denoise_video=true`. The Loaders use the INT8
model names from the documentation. If your local installation uses BF16 shards
or different logical names, select the matching entries from the dropdowns.
...
1. **Model names**: The three Loaders' `model_root` values and component dropdowns
   ...
   repository's root README for weight placement rules.
