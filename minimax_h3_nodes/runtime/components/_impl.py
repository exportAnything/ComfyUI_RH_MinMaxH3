...
def weights_root_paths(
    ...
    *,
    ...
    roots: list[Path] = []
    raw_extra = os.environ.get(H3_WEIGHTS_ROOTS_ENV, "")
    if raw_extra:
        roots.extend(
    ...
    roots.append(global_models / WEIGHTS_ROOT_DIRNAME)
    return [path for path in _unique_paths(roots) if path.is_dir()]


def list_h3_weight_files(
    kind: str | None = None,
    ...
    if wanted_kind is not None and wanted_kind not in H3_COMPONENT_KINDS:
        raise H3ComponentError(
            f"Unknown MiniMax H3 component kind {kind!r}; expected one of "
            f"{H3_COMPONENT_KINDS!r}"
        )
    wanted_partition = (
        _normalize_partition(partition) if partition is not None else None
    )
    out: list[Path] = []
    seen: set[str] = set()
    for root in weights_root_paths(kind=wanted_kind):
        ...
        for child in children:
            if not child.is_file():
                continue
            classified = classify_weight_filename(child.name)
            ...
            if (
                ...
                and file_partition != wanted_partition
            ):
                continue
...
def weight_file_partition(path: str | Path) -> str | None:
    """Return the partition a flat weight file declares through its name."""

    classified = classify_weight_filename(Path(path).name)
    return classified[1] if classified is not None else None


def validate_weight_partition(
    path: str | Path,
    partition: str,
    *,
    kind: str,
) -> Path:
    """Reject a flat weight file that names the other partition.

    The flat root carries no ``quant_meta.json``, so the converter naming
    convention is the partition proof.  A file that declares no partition
    (text encoder, VAEs) legitimately serves both.
    """

    resolved = Path(path).expanduser().resolve()
    normalized = _normalize_partition(partition)
    classified = classify_weight_filename(resolved.name)
    if classified is None:
        raise H3ComponentError(
            f"MiniMax-H3 weight file {resolved.name!r} does not follow the "
            "converter naming convention and carries no partition proof"
        )
    file_kind, declared = classified
    if file_kind != str(kind).strip().lower():
        raise H3ComponentError(
            f"MiniMax-H3 weight file {resolved.name!r} is a {file_kind!r} "
            f"checkpoint, not {kind!r}"
        )
    if declared is not None and declared != normalized:
        raise H3ComponentError(
            f"Requested {normalized!r} weights, but {resolved.name!r} declares "
            f"partition {declared!r}"
        )
    return resolved
