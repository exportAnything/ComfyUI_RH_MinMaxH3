...
from ._compat import logging, maybe_allow_in_graph
...
class RotaryEmbeddingND(nn.Module):
    def __init__(self, dim, rotary_base=10000, n_dim=3, use_angle=False):
        ...
        # The low-memory VAE loader constructs modules under a meta-device
        # context.  This derived, non-persistent buffer has no checkpoint key
        # that could materialize it later, so keep its tiny canonical value on
        # CPU and move it to the input device lazily in ``forward``.
        inv_freq = 1 / rotary_base ** torch.arange(
            0, 1, 2 * n_dim / dim, dtype=torch.float32, device="cpu"
        )
