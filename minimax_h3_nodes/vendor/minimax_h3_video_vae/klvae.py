...
from .parallel import all_gather, all_gather_var_shape, get_parallel_state
...
class AutoencoderKL(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def _all_gather_tiled_results(self, tasks, num_tiles):
        ...
        expected_tasks = (num_tiles - sp_rank + sp_size - 1) // sp_size
        if len(tasks) != expected_tasks:
            raise ValueError(
                f"Expected {expected_tasks} tiled tasks on rank {sp_rank}, "
                f"got {len(tasks)} for num_tiles={num_tiles}, sp_size={sp_size}"
            )

        stacked = torch.stack(tasks, dim=0)
        max_tasks = (num_tiles + sp_size - 1) // sp_size
        ...
        # num_tiles and sp_size. Pad only the leading task dimension and use a
        ...
        # clip.
        gathered = all_gather(stacked, group=group)

        results = [None] * num_tiles
        for rank, rank_tensors in enumerate(gathered):
            num_rank_tasks = (num_tiles - rank + sp_size - 1) // sp_size
