...
from ._compat import ConfigMixin, ModelMixin, logging, register_to_config
...
class ViTBase(ModelMixin, ConfigMixin):
    ...
    def set_spatial_parallel(self, enabled):
        ...
        if hasattr(self, "transformer_blocks"):
            for block in self.transformer_blocks:
                block.attn.set_spatial_parallel(enabled)
    ...
    def _pad_for_sp(self, hidden_states, img_ids, pack_info=None):
        ...
        # The sequence must be evenly divisible for the sequence-to-head
        # all-to-all. Keep the true length as metadata instead of materializing
        # an O(S^2) padding mask; Attention slices Q/K/V back to this length
        # while computing global attention, then restores dummy rows solely for
        # the reverse all-to-all.
        pack_info = dict(pack_info)
        pack_info["valid_seq_len"] = seq_len
...
class ViT3DDecoder(ViTBase):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.spatial_parallel:
            ...
            sp_process_group = parallel_state["sp_process_group"]
            hidden_states = get_subseq(hidden_states, sp_size=sp_size)
        ...
        if self.spatial_parallel:
            hidden_states = gather_subseq(
                hidden_states,
                sp_size=sp_size,
                local_process_group=sp_process_group,
            )
