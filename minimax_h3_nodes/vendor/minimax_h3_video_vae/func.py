...
def apply_rotary_pos_emb_qk(
    ...
    """Apply the vendored PyTorch NeoX rotary implementation to Q and K.

    The source runtime optionally calls ``sgl_kernel.rotary_embedding`` here.
    A ComfyUI custom-node installation must not acquire that compiled SGLang
    dependency, so the mathematically equivalent local PyTorch path is always
    used.
    """
    return (
